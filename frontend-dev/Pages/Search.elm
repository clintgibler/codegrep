    ...
    case msg of
        ...
        RepositoryInputChanged newRepositoryInput ->
            let
                ...
                newInput =
                    { input | repository = Just newRepositoryInput }
            in
        ...
        ChangeSearchInput newSearchInput ->
            let
                ...
                newInput =
                    { input | query = Just newSearchInput }
            in
                ( { model | input = newInput }, Cmd.none )
        ...
        ChangeLanguage newLanguage ->
            let
                ...
                newInput =
                    { input | language = Just newLanguage, identifier = Just "any" }
        ...
        ChangeIdentifier newIdentifier ->
            let
                ...
                newInput =
                    { input | identifier = Just newIdentifier }
            in
                ( { model | input = newInput }, Cmd.none )
...
init model maybeQuery maybeLanguage maybeIdentifier maybeRepository =
    let
        newInput =
            { query = maybeQuery, language = maybeLanguage, identifier = maybeIdentifier, repository = maybeRepository }
                ...
                case model.input.identifier of
                    ...
                    Just t ->
                        if t == "any" then
                            ...
                            url ++ "?tokens.type=" ++ t ++ "&tokens.text=" ++ query
           )
        |> (\url ->
                case model.input.language of
                    Nothing ->
        ...
        div [ class "black-80 pa4 bg-washed-yellow" ]
            [ div [ class "flex-ns flex-wrap-ns" ]
                ...
                , div [ class "w-40-ns pr2 pb2" ]
                    [ input
                        [ class "input-reset ba b--black-20 w-100 pa2"
                        , id "repository"
                ...
                , div [ class "w-20-ns pr2 pb2" ]
                    [ select [ class "select w-100", on "change" (Decode.map ChangeLanguage Html.Events.targetValue) ] (maybeSearchLanguageOptions model)
                    ]
                , div [ class "w-20-ns pr2 pb2" ]
                    [ select [ class "select w-100", on "change" (Decode.map ChangeIdentifier Html.Events.targetValue) ] (maybeSearchIdentifierOptions model)
                    ]
                ]
            , div [ class "flex-ns flex-wrap-ns" ]
...
maybeSearchIdentifierOptions : Model -> List (Html Msg)
maybeSearchIdentifierOptions model =
    case model.identifiers of
        RemoteData.NotAsked ->
        ...
        RemoteData.Success identifiers ->
            case model.input.identifier of
                Just lang ->
                    option [ value "any" ] [ text "Identifier" ] :: searchIdentifierOptions model identifiers
                Nothing ->
                    option [ value "any", selected True ] [ text "Identifier" ] :: searchIdentifierOptions model identifiers

        RemoteData.Failure error ->
            [ option [ value "any" ] [ text "Identifier" ] ]


searchIdentifierOptions : Model -> List String -> List (Html Msg)
searchIdentifierOptions model identifiers =
    List.map
        (\x ->
            case model.input.identifier of
                Just iden ->
                    if iden == x then
                        option [ selected True ] [ text x ]
                        else
                            option [ ] [ text x ]
...
maybeSearchLanguageOptions : Model -> List (Html Msg)
maybeSearchLanguageOptions model =
    case model.languages of
        ...
        RemoteData.Success languages ->
            option [ value "any" ] [ text "Language" ] :: searchLanguageOptions model languages

        RemoteData.Failure error ->
            [ option [ value "any" ] [ text "Language" ] ]


searchLanguageOptions : Model -> List String -> List (Html Msg)
searchLanguageOptions model languages =
    List.map
        (\x ->
            case model.input.language of
                Just lang ->
